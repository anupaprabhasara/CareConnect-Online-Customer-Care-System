...
<aside class="fixed top-0 left-0 w-60 hidden md:flex flex-col bg-white border-r border-blue-200 shadow-md h-screen z-20">
  <!-- Logo -->
  <div class="p-6 text-center border-b border-blue-200">
    <h1 class="text-2xl font-bold text-blue-600">CareConnect</h1>
    <c:if test="${not empty sessionScope.role}">
      <p class="mt-1 text-sm text-gray-500">Your role:
        <span class="font-semibold text-blue-500">${sessionScope.role}</span>
      </p>
    </c:if>
  </div>
  ...
  <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto text-sm font-medium">
    ...
    <c:if test="${sessionScope.role == 'Agent'}">
      <a href="${pageContext.request.contextPath}/admin/ticket"
        class="flex items-center p-3 rounded hover:bg-blue-100 transition text-gray-700 hover:text-blue-600">
